...
sub _check_manifest_files {
    my ($dist) = @_;
    my $skip = do {
        open my $fh, '<', "$dist/MANIFEST.SKIP"
            or die "Unable to read MANIFEST.SKIP: $!";
        local $/;
        <$fh>;
    };
    like($skip, qr/^#!include_default$/m, 'MANIFEST.SKIP includes default rules');
    ok((grep { $_ eq '\.tar\.gz\z' } split /\n/, $skip),
        'MANIFEST.SKIP excludes release archives');
    ok((grep { $_ eq '^Net-Blossom.*-\d' } split /\n/, $skip),
        'MANIFEST.SKIP excludes distribution directories');
